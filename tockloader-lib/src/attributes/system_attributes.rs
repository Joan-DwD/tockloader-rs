...
impl SystemAttributes {
    ...
    pub(crate) async fn read_system_attributes_serial(
        ...
        // Send read command over serial, expecting a ReadRange response of 1024 bytes
        let (_, buf) = issue_command(
            port,
            Command::ReadRange, // Read memory (0x10)
        ...
        // Issue ReadRange command for bootloader version
        let (_, buf) =
